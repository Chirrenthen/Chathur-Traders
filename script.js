...
function renderInventory() {
  ...
  inventory.forEach(p => {
    ...
    tr.innerHTML = `
      <td>CT-${p.id.toString().slice(-4)}</td>
      <td>${p.name}</td>
      <td>${p.description || '-'}</td>
...
function renderBills() {
  ...
  bills.forEach(b => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>CT-${b.id}</td><td>${b.date}</td><td>${b.customer}</td><td>₹${b.total.toFixed(2)}</td><td>₹${b.paid.toFixed(2)}</td><td>₹${b.pending.toFixed(2)}</td><td>${b.status}</td>`;
...
function updateDashboard() {
  ...
  recent.forEach(b => {
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>CT-${b.id}</td><td>${b.customer}</td><td>${b.date}</td><td>₹${b.total.toFixed(2)}</td><td>${b.status}</td>`;
...
/* -------- Export implementation (final) --------
   - Title (merged A1:G1): (Month / Year) - Chathur Traders
...
function exportMonthlyData(month, year) {
  ...
  const title = `(${monthName} / ${year}) - Chathur Traders`;
  ...
  const filename = `(${monthName} / ${year}) - Chathur Traders.xlsx`;
